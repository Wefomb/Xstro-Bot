...
const regexSend = new RegExp(
 `\\b(?:${['send', 'share', 'snd', 'give', 'save', 'sendme', 'forward'].join('|')})\\b`,
 'i'
)
smd({ on: 'quoted' }, async (message, text) => {
 try {
  ...
  if (mm && regexSend.test(text.toLowerCase())) {
   message.bot.forwardOrBroadCast(message.fromMe ? message.user : message.from, mm, {
    quoted: { key: mm.key, message: mm.message },
   })
  }
 } catch (e) {
...
global.waPresence =
 process.env.WAPRESENCE && process.env.WAPRESENCE === 'online' ? 'available' : process.env.WAPRESENCE || ''
...
smd({ on: 'status' }, async (message, text) => {
 try {
  if (
   `${global.read_status_from}`.split(',').includes(message.key.participant.split('@')[0]) ||
  ...
  if (
   (`${global.save_status_from}`.split(',').includes(message.key.participant.split('@')[0]) ||
