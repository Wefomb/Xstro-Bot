...
smd(
 ...
 async (message, input) => {
  try {
   ...
   console.error(error)
   return message.error(error + '\n\nCommand: wanews', error, '*Failed to fetch WhatsApp beta news.*')
  }
 }
)
 ...
 async m => {
  try {
   ...
   if (!data || !data.results || data.results.length === 0) {
    return await m.send('*Failed to fetch articles from Spaceflight News API.*')
   }
